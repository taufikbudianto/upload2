 ...
 * @author Ismail
 */
public class MapCalculation implements java.io.Serializable {
...
//    private static SimpleDateFormat sdfHHmm = new SimpleDateFormat("HH:mm");
    ...
    /**
     * methode ini untuk mendapatkan titik pusat dari titik-titik coordinat
     * sphasial
     *
     * @param points titik-titik coordinat yang akan
     * @return titik pusat dari titik-titik coordinat
     */
    public static LatLng getPivotFromMuchCoodninate(List<LatLng> points) {
...
//        Double totalHours = hours + ((double) minutes / (double) 60) + ((double) seconds / (double) 3600) + ((double) milliseconds / (double) 3600000);
    ...
    public static List<Waypoint> waypoints(List<LatLng> paths) {
        List<Waypoint> listWaypoint = new ArrayList<>();
        int seq = 1;
        ...
        for (LatLng ll : paths) {
            ...
            Double distance = 0.0;
            if (sizePath > seq) {
                distance = MapCalculation.calculateDistanceInKm(
                        ll.getLat(), ll.getLng(), paths.get(seq).getLat(), paths.get(seq).getLng());
                distance = DecimalUtil.rounding(distance);
            }
            w.setDistance(distance);
            w.setLatitude(String.valueOf(ll.getLat()));
            w.setLongitude(String.valueOf(ll.getLng()));
            w.setSequence(seq++);
            listWaypoint.add(w);
        }
        return listWaypoint;
