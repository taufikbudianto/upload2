 ...
 */
GEarthExtensions.prototype.edit.editLineString = function(lineString, isDblClickRemoveReguralPlacemark, 
                                                          options) {
  options = checkParameters_(options, false, {
    editCallback: ALLOWED_,
	dropCallback: ALLOWED_,
	deleteCallback: ALLOWED_
  });
  isDblClickRemoveReguralPlacemark = isDblClickRemoveReguralPlacemark || false;
  var lineStringEditData = this.util.getJsDataValue(
      lineString, LINESTRINGEDITDATA_JSDATA_KEY) || {};
  if (lineStringEditData) {
    this.edit.endEditLineString(lineString);
  ...
  var makeRegularDeleteEventListener_ = function(coordData) {
    return function(event) {
	  ...
	  for(i = 0 ; i < numCoords ; ++i){
		coord = coords.get(i);
		l.push(new GLatLng(coord.getLatitude(), coord.getLongitude()));
	  }
	  if(options.deleteCallback){
		options.deleteCallback(coordData.index, l);
	  }
    };
  };
  
  var makeRegularDropCallback_ = function(coordData){
	return function(){
		var index, latlng, coord;
		index = coordData.index;
		coord = coords.get(index);
		latlng = new GLatLng(coord.getLatitude(), coord.getLongitude());
		if(options.dropCallback){
			options.dropCallback(index, latlng);
		}
	}
  }
  ...
  me.util.batchExecute(function() {
    for (var i = 0; i < numCoords; i++) {
	  ...
	  coordData.regularDropCallback = makeRegularDropCallback_(coordData);
      ...
      me.edit.makeDraggable(coordData.regularPlacemark, {
		...
		dropCallback: coordData.regularDropCallback
      });

	  if(isDblClickRemoveReguralPlacemark){
		coordData.deleteEventListener = makeRegularDeleteEventListener_(coordData);
		google.earth.addEventListener(coordData.regularPlacemark, 'dblclick',coordData.deleteEventListener);
	  }
    }
