...
//    public final static String DESC_TILT = "Tilt";
    ...
    /**
     * Membuat Object GTaskSearchArea baru berdasarkan String area yang mendeskripsikan 
    ...
    /**
     * Interface for creating task search area which come from Trapezium TaskArea
     * @see TrapeziumCalculationController
     */
    public static void interfaceCreateTaskAreaComeFromTrapezium(TrapeziumTaskArea tta){
    //public static void interfaceCreateTaskAreaComeFromTrapezium(TrapeziumTaskArea tta){    
        try {
            List<GLatLng> l = new ArrayList<GLatLng>();
            l.add(GLatLng.toJava(tta.getSmallCoord1()));
            l.add(GLatLng.toJava(tta.getSmallCoord2()));
            l.add(GLatLng.toJava(tta.getBigCoord2()));
            l.add(GLatLng.toJava(tta.getBigCoord1()));
            //Mendapatkan titik tengah dari 4 Coordinate
            GLatLng pivot = GLatLng.getPivotFromMuchCoodninate(l);
    ...
    /**
     * interface for remove task area one or more in navigator. which have TrapeziumTaskArea id equals with some of Collection in 
     * <code>ids</code> parameter
     * 
     * @see TrapeziumCalculationController
     */
