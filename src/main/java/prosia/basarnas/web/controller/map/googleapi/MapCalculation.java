...
import prosia.basarnas.model.MstResource;
...
public class MapCalculation {
     ...
     * @return double jarak dari kedua object berdasarkan permukaan bumi yang
     * rata
     */
    ...
    /**
     * Mengconvert bentuk Kilometer menjadi String dengan format "kmValue .
     * mValue KM" contoh : dengan parameter 192.9281934249 akan mengembalikan
     * return 192.928 KM;
     *
    ...
    /**
     * untuk mengetahui apakah resource masuk dalam radius yang ditentukan dari
     * incedent
     *
     ...
     *
     * @return true jika resource termasuk dalam radius yang ditentukan, false
     * jika sebaliknya
     */
    public boolean valid(MstResource r, double latInc, double lngInc, double rad) {
...
//        Double totalHours = hours + ((double) minutes / (double) 60) + ((double) seconds / (double) 3600) + ((double) milliseconds / (double) 3600000);
    ...
    public static LatLong rotation(LatLong poros, LatLong pi, double degree) {
        degree = otherwise(degree);
        double x_ = poros.getLongitude(), y_ = poros.getLatitude(),
                x = pi.getLongitude(), y = pi.getLatitude(),
                xGenerate, yGenerate,
                cos, sin;
        ...
        LatLong result = new LatLong(yGenerate, xGenerate);
        return result;
    }

    public static double getAngle(LatLong pivot, LatLong titikAwal, LatLong titikAkhir) {
        double dpoX = titikAwal.getLongitude() - pivot.getLongitude();
        double dpoY = titikAwal.getLatitude() - pivot.getLatitude();
        double dpiX = titikAkhir.getLongitude() - pivot.getLongitude();
        double dpiY = titikAkhir.getLatitude() - pivot.getLatitude();
