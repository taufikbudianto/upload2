/*
	...
	Modified to perform a new action on a cluster marker click
	
	All modifications commented with
	//	modification:
...
function ClusterMarker($map, $options){
    ...
    if($options.clusterMarkerIcon){
        ...
        this.clusterMarkerIcon.image=CLUSTER_MARKER_ICON_ARROW;
        ...
        this.clusterMarkerIcon.shadow=CLUSTER_MARKER_ICON_SHADOW_ARROW;
        this.clusterMarkerIcon.shadowSize=new GSize(39, 34);
    }
    this.clusterMarkerTitle=($options.clusterMarkerTitle)?$options.clusterMarkerTitle:'%count markers here, click for more information';
    ...
    //	modification: a new property that if set to false will cause ClusterMarker to stop refreshing itself
    //	a refresh when a cluster marker or clustered marker infowindow is open would cause the infowindow to close, the clustered marker to be clustered and the cluster marker would re-appear
    //	an 'infowindowclose' event is used to set refreshEnabled back to true to cause normal functioning of ClusterMarker
...
ClusterMarker.prototype._clusterMarker=function($clusterGroupIndexes){
    ...
    var hasIncidentImage = false;
    for(i=$clusterGroupIndexes.length-1; i>=0; i--){
        $mapMarkers[$clusterGroupIndexes[i]]._parentCluster=$clusterMarker;
        if(!hasIncidentImage){
            if($mapMarkers[$clusterGroupIndexes[i]].getTitle().indexOf("(Incident)") != -1 ||
            $mapMarkers[$clusterGroupIndexes[i]].getTitle().indexOf("(Beacon Elemental)") != -1 ||
            $mapMarkers[$clusterGroupIndexes[i]].getTitle().indexOf("(Beacon Composite GPS)") != -1 ||
            $mapMarkers[$clusterGroupIndexes[i]].getTitle().indexOf("(Beacon Composite Doppler)") != -1){
                //$clusterMarker diset menjadi true sebagi indikasi bahwa clusterMarker tersebut mempunyai incident POI
                //dan code ini akan dijadikan flag untuk menjalankan method setImage
                //dan ketika setImage sudah dipanggil maka clusterMarker akan di disableDragging()
                $clusterMarker.enableDragging();
                hasIncidentImage = true;
            }else{
                $clusterMarker.disableDragging();
            }
        }
    }
...
ClusterMarker.prototype.clusterMarkerClick=function($args){
    //	see source code for new method
};
    ...
    for(i=$mapMarkers.length-1; i>0; i--)
    {
        if($mapMarkers[i]._makeVisible){
            ...
            if($clusterGroup.length!==0){
                //	modification: required to ensure clustered markers in array maintain their original order
                $clusterGroup.reverse();
                $clusterGroup.push(i);
                $clusterGroup.reverse();
...
ClusterMarker.prototype.refresh=function($forceFullRefresh){
    //	modification: only perform a refresh if the new refreshEnabled property is true
    if(this.refreshEnabled){
        ...
        if(this._activeMarkersChanged || $forceFullRefresh){
            ...
            //	modification: perform clustering at all zoom levels - even if the map is fully zoomed in
            //	the new clusterMarkerClick() method will handle markers at the same location
            //	(that is markers that don't uncluster no matter how much the map is zoomed in)
            if(this.clusteringEnabled && this.refreshEnabled){
            ...
            for(i=this._clusterMarkers.length-1; i>=0; i--){
                ...
                if(this._clusterMarkers[i].draggingEnabled()){
                    this._clusterMarkers[i].setImage(INCIDENT_CLUSTER_MARKER_ICON_ARROW);
                    this._clusterMarkers[i].disableDragging();
                }
            }
...
ClusterMarker.prototype.removeMarker = function(id){
    var $mapMarkers=this._mapMarkers, $map=this._map;
    if($mapMarkers[id]._isVisible){
        $map.removeOverlay($mapMarkers[id]);
    }
    delete $mapMarkers[id]._isVisible;
    delete $mapMarkers[id]._isActive;
    delete $mapMarkers[id]._makeVisible;
    this._removeClusterMarkers();
    this._mapMarkers = wasteElement(this._mapMarkers, id);
    this._iconBounds = wasteElement(this._iconBounds, id);
};
