...
SearchArea.prototype.add = function(latlng){
    if(this.polygon != null){
        if(this.polygon.getBounds().containsLatLng(latlng)) //Jika latlng berada dalam area polygon
            return;
    }
    this.l.push(latlng);
    this.install();
...
SearchArea.prototype.install = function(){
    if(this.l.length > 2){
        ...
        if(this.shape == 'Circle'){
            this.polygon = new Circle(getPivotPoint(this.l), this.radius).poly;
        }else{
            ...
            //Disable Editing Polygon for Fusion Table
            //this.polygon.enableEditing({maxVertices : this.polygon.getVertexCount()});
            var me = this;
            GEvent.addListener(this.polygon, "mouseover", function(){
                //TODO show Tooltip
                });
            GEvent.addListener(this.polygon, "lineupdated", function(){
                // Jika suatu Vertex di edit Posisinya
                //Set nilai baru dari properti l
                me.doAfterLineUpdate(this, me.l);
            });
...
SearchArea.prototype.doAfterLineUpdate = function(poly, l){
    this.l = new Array();
    this.l.splice((count-1), count);
    var count = poly.getVertexCount();
    //properti l menampung semua vertex yang dimiliki pada properti polygon
    for(i=0;i<count;i++){
        var vertex = poly.getVertex(i);
        this.l.push(vertex);
    }
    
    //Membuat nilai l menjadi tidak mengandung index terakhir(Vertex Awal yang kedua)
    this.l.splice((count-1), count);
}
